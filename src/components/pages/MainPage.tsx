...
const MainPage = (): JSX.Element => {
    const { loading, error, data, refetch } = useQuery<TodoData, TodoVars>(GET_TODOS, {
        variables: {
            options,
        },
    });
    ...
    const handleStatus = (item: Todo): void => {
        ...
        if (sortType === SortType.TO_DO) {
            return setSortedData((prevState) => ({
                important: prevState.important.filter((todo) => todo.id !== item.id),
        ...
        if (sortType === SortType.DONE) {
            return setSortedData((prevState) => ({
                important: [...prevState.important, { ...item, completed: true }],
    ...
    const handleConfirmModal = (item: Todo) => {
        if (item.id) {
            setSortedData((prevState) => {
                ...
                return getSortedItemsByStatus([...items.important, ...items.items], sortType);
            });
        } else {
            if (item.completed) {
                setSortedData((prevState) => ({
                    important: [{ ...item, id: getNewTodoId() }, ...prevState.important],
                    items: prevState.items,
                }));
            } else {
                setSortedData((prevState) => ({
                    important: prevState.important,
                    items: [{ ...item, id: getNewTodoId() }, ...prevState.items],
                }));
            }
        }
    };
    ...
    const handleSearch = (e: React.ChangeEvent<HTMLInputElement>): void => {
        ...
        const result = items?.reduce((acc: Todo[], todo: Todo) => {
            if (isTitleContained(todo.title, searchValue)) {
                ...
                acc.push({
                    ...todo,
                    title: todo.title.replace(tSearchValue, `**${tSearchValue}**`),
                });
    ...
    const activateLazyLoading = ({ refObserverContainer, refItemsContainer }: lazyLoadingProps) => {
        if (refItemsContainer.current && refObserverContainer.current) {
            ...
            const anchorElement: Element = itemsContainer.lastElementChild as Element;
    ...
    useEffect(() => {
        if (searchData) {
            setSearchData(
                getSortedItemsByStatus([...searchData.important, ...searchData.items], sortType),
            );
        } else {
            setSortedData(
                getSortedItemsByStatus([...sortedData.important, ...sortedData.items], sortType),
            );
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [sortType]);
            ...
            <div className="main-page" ref={refWrapper}>
                <div className="main-page__actions-container">
                    <div className="main-page__description-container">
                        <h1 className="main-page__title">To Do</h1>
                        <span className="main-page__subtext">This is your todos</span>
                    </div>
                ...
                <div className="main-page__table-container">
                    ...
                    />
                    {smallLoader && <div className="spinner main-page__small-loader" />}
                </div>
            </div>
            {todoModal && (
                <TodoModal
